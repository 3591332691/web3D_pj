...
async function getExamID() {
    const baseUrl = 'http://150.158.39.251:8000/exam/add';
    try {
...
async function getExamQuestions(examID) {
    const baseUrl = 'http://150.158.39.251:8000/exam/questions';
...
async function submitExamAnswers(examID, options) {
    const baseUrl = 'http://150.158.39.251:8000/exam/submit';
    const url = baseUrl;
...
async function getAIAnswer(question) {
    const baseUrl = 'http://150.158.39.251:8000/ai';
...
/*		this.guide.addEventListener('click', function() {
	...
	submitExam1(examID, questions) {
		...
		// 向后端发送提交请求
		fetch('http://150.158.39.251:8000/exam/submit', {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json',
			},
